...
for(y in 2003:2017){
    ...
    mod = mgcv::gam(obs_anamoly ~  ukmo_anamoly, data = DT_train)
    mod2 = mgcv::gam(obs_anamoly ~  ecmwf_anamoly, data = DT_train)
    mod3 = mgcv::gam(obs_anamoly ~  ecmwf_anamoly + ukmo_anamoly, data = DT_train)

    DT_final[year == y,
             ukmo := climatology + predict(mod,newdata = DT_final[year == y])]
    DT_final[year == y,
             ecmwf := climatology + predict(mod2,newdata = DT_final[year == y])]
    DT_final[year == y,
             multi_model := climatology + predict(mod3,newdata = DT_final[year == y])]
}
