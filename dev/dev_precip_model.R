...
load("./FcNov2018/prect_hindcast_slimmed.RData")

ens_names = c("norcpm_prect_bar", "ecmwf_prect_bar")
...
for(y in 2006:2017){
    ...
    mod = lm(obs_anamoly ~  climatology + obs_anamoly_1 + norcpm_anamoly + ecmwf_anamoly, data = DT_train)
    DT_final[year == y,
...
if(print_figs){pdf(paste0(path_out,"/Model_to_Basic_prec.pdf"))}else{X11()}
rr = range(Score[,-1])
plot(1:12,Score[[2]], type  = "l", xlab = "Month", ylab = "RMSE", ylim = rr)
