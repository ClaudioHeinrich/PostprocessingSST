...
for(y in 2006:2017){
    print(y)
    X = as.matrix(DT_final[year < y, .(Lon, Lat, month)])
    Y = as.matrix(DT_final[year < y, residual])
    X_test = as.matrix(DT_final[year == y, .(Lon, Lat, month)])
    qrf <- quantregForest(x = X, y = Y, nthread = 30)
    p_hat = predict(qrf, X_test, what=0.05)
    DT_final[year == y, Q_pred := p_hat]
}

DT_plot = square_image(DT_final[,mean(residual < Q_pred,na.rm=TRUE), .(Lon,Lat)])
plot_smooth2(DT_plot, rr = c(0,.1),set_white = .05)
