...
for(y in 2010:2017){
    ...
    print(y)
    DT_train = DT_final[year < y]
    DT_train[,var_global := var(residual,na.rm =TRUE)]
    DT_train[,var_grid := var(residual, na.rm = TRUE),grid_id]
    DT_train[,var_month := var(residual, na.rm = TRUE),month]
    DT_train[,var_month_grid := var(residual, na.rm = TRUE),.(month,grid_id)]
    DT_grid = DT_train[,tail(.SD,1),.(grid_id,month),.SDcols = nms_run]
    DT_grid[,year:=y]
    DT_train_list[[y]] = DT_grid
}

DT_var = rbindlist(DT_train_list)

DT_q = merge(DT_final[,.(grid_id, month, year,Lon,Lat,obs_erai_ts,trained_model)],
             DT_var[,.SD,.SDcols=c("grid_id","month","year",nms_run)],
             by = c("grid_id", "month", "year"))
             
for(j in 1:length(nms_run)){
    DT_q[,eval(paste0("Q_",nms_run[j])) := qnorm(.05,trained_model,get(eval(nms_run[j])))]
}

rr_all = c(0,.1)

DT_plot = square_image(DT_q[,mean(obs_erai_ts < Q_var_month_grid),.(Lon,Lat)])
if(print_figs){pdf(paste0(out_path,"q_valid_month_grid.pdf"))}else{X11()}
