...
#' @param nPCs the max number of principal components to consider.
  ...
  forecast_by_month = function(m)
    {
      ...
      for(y in Y)
      {
        ...
        eigen_vectors <- PCA$u[,1:nPCs[which(M == m)]]
        sqrt_sing_values <- sqrt(diag(x = PCA$d[1:nPCs[which(M == m)]], nrow = length(PCA$d[1:nPCs[which(M == m)]]))) # the awkward notation ensures that diag does the right thing if nPCs = 1.
        ...
        # generate noise
        no = var_correct_mat %*% eigen_vectors  %*% sqrt_sing_values %*% matrix(rnorm(nPCs[which(M == m)] * n),nrow = nPCs[which(M == m)], ncol = n)
                                 ...
                                 mc_cores = length(M))
{
  ...
  forecast_by_month = function(m)
  {
    ...
    for(y in Y)
    {
      ...
      no_1 = eigen_vectors  %*% sqrt_sing_values %*% matrix(rnorm(nPCs[which(M == m)] * n),nrow = nPCs[which(M == m)], ncol = n)
