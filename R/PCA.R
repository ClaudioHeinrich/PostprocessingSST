...
#' computes the sample covariance matrices downweighted by distance of gridpoints
#'
#' @param dt The data table
...
sam_cov = function(dt, weight_mat,
                   Y=unique(dt[,year]), M = 1:12, 
                   save_years = 2013:2018, save_dir, file_name = "sam_cov")
{ 
  ...
  # get for each month the normalized data matrix, i.e. the root covariance matrix
  ...
  for(m in M)
  {
    ...
    for(y in save_years)
    {
      ...
      sam_cov_mat = 1/sqrt(num_years) * full_data_mat[,1:num_years] %*% t(full_data_mat[,1:num_years]) 
  ...
  forecast_by_month = function(m)
    {
      ...
      for(y in Y)
      {
        ...
        PCA = irlba::irlba(Sigma, nv = nPCs) 
        
        
        eigen_vectors <- PCA$u[,1:nPCs]
        sing_values <- diag(x = PCA$d[1:nPCs], nrow = length(PCA$d[1:nPCs])) # the awkward notation ensures that diag does the right thing if nPCs = 1.
        ...
        # generate noise
        no = var_correct_mat %*% eigen_vectors  %*% sing_values %*% matrix(rnorm(nPCs * n),nrow = nPCs, ncol = n)
